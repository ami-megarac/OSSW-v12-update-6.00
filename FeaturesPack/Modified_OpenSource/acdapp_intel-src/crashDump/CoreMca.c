...
static void coreMcaJson(UINT8 u8Cpu, UINT32 u32CoreNum, SCoreMcaRawData * sCoreMcaRawData, cJSON * pJsonChild)
{
    ...
    for (i = FIRST_CORE_MCA; i <= LAST_CORE_MCA; i++) {
		...
		if (sCoreMcaRawData[i].bInvalid) {
			snprintf(jsonItemString, CORE_MCA_JSON_STRING_LEN, "%s0x%02x", UA_STRING, sCoreMcaRawData[i].uRegData.u32Raw[0]);
		...
		if (sCoreMcaRawData[i].u8CC[0] == PECI_CC_SUCCESS) {
			snprintf(jsonItemString, CORE_MCA_JSON_STRING_LEN, "0x%08x%08x",
				sCoreMcaRawData[i].uRegData.u32Raw[1], sCoreMcaRawData[i].uRegData.u32Raw[0]);
		}
		else {
			snprintf(jsonItemString, CORE_MCA_JSON_STRING_LEN, "0x%08x%08x,%s0x%02x",
				sCoreMcaRawData[i].uRegData.u32Raw[1], sCoreMcaRawData[i].uRegData.u32Raw[0],
				UA_STRING, sCoreMcaRawData[i].u8CC[0]);
		}
		cJSON_AddStringToObject(coreMca, coreMcaRegNames[CORE_CTL], jsonItemString);

		if (sCoreMcaRawData[i].u8CC[1] == PECI_CC_SUCCESS) {
			snprintf(jsonItemString, CORE_MCA_JSON_STRING_LEN, "0x%08x%08x",
				sCoreMcaRawData[i].uRegData.u32Raw[3], sCoreMcaRawData[i].uRegData.u32Raw[2]);
		}
		else {
			snprintf(jsonItemString, CORE_MCA_JSON_STRING_LEN, "0x%08x%08x,%s0x%02x",
				sCoreMcaRawData[i].uRegData.u32Raw[3], sCoreMcaRawData[i].uRegData.u32Raw[2],
				UA_STRING, sCoreMcaRawData[i].u8CC[1]);
		}
        cJSON_AddStringToObject(coreMca, coreMcaRegNames[CORE_STATUS], jsonItemString);

		if (sCoreMcaRawData[i].u8CC[2] == PECI_CC_SUCCESS) {
			snprintf(jsonItemString, CORE_MCA_JSON_STRING_LEN, "0x%08x%08x",
				sCoreMcaRawData[i].uRegData.u32Raw[5], sCoreMcaRawData[i].uRegData.u32Raw[4]);
		}
		else {
			snprintf(jsonItemString, CORE_MCA_JSON_STRING_LEN, "0x%08x%08x,%s0x%02x",
				sCoreMcaRawData[i].uRegData.u32Raw[5], sCoreMcaRawData[i].uRegData.u32Raw[4],
				UA_STRING, sCoreMcaRawData[i].u8CC[2]);
		}
		cJSON_AddStringToObject(coreMca, coreMcaRegNames[CORE_ADDR], jsonItemString);

		if (sCoreMcaRawData[i].u8CC[3] == PECI_CC_SUCCESS) {
			snprintf(jsonItemString, CORE_MCA_JSON_STRING_LEN, "0x%08x%08x",
				sCoreMcaRawData[i].uRegData.u32Raw[7], sCoreMcaRawData[i].uRegData.u32Raw[6]);
		}
		else {
			snprintf(jsonItemString, CORE_MCA_JSON_STRING_LEN, "0x%08x%08x,%s0x%02x",
				sCoreMcaRawData[i].uRegData.u32Raw[7], sCoreMcaRawData[i].uRegData.u32Raw[6],
				UA_STRING, sCoreMcaRawData[i].u8CC[3]);
		}
		cJSON_AddStringToObject(coreMca, coreMcaRegNames[CORE_MISC], jsonItemString);

		if (sCoreMcaRawData[i].u8CC[4] == PECI_CC_SUCCESS) {
			snprintf(jsonItemString, CORE_MCA_JSON_STRING_LEN, "0x%08x%08x",
				sCoreMcaRawData[i].uRegData.u32Raw[9], sCoreMcaRawData[i].uRegData.u32Raw[8]);
		}
		else {
			snprintf(jsonItemString, CORE_MCA_JSON_STRING_LEN, "0x%08x%08x,%s0x%02x",
				sCoreMcaRawData[i].uRegData.u32Raw[9], sCoreMcaRawData[i].uRegData.u32Raw[8],
				UA_STRING, sCoreMcaRawData[i].u8CC[4]);
		}
		cJSON_AddStringToObject(coreMca, coreMcaRegNames[CORE_CTL2], jsonItemString);
    }
}
...
if (sRdPkgConfigRes.u8CompletionCode == PECI_CC_SUCCESS) {
			...
			// chop of the thread count on 0x91/0x94 read if exceed the max
			if (sRdPkgConfigRes.u8CompletionCode == PECI_CC_MAYBE_SUCCESS || sRdPkgConfigRes.u8CompletionCode == PECI_CC_CMD_PASSED) {
				if (u32MaxThreadID > MAX_THREAD_ID)
					u32MaxThreadID = MAX_THREAD_ID;

				PRINT(PRINT_DBG, PRINT_INFO, "[DBG] - CPU #%d has %ld threads (cc=0x%x)\n", u8Cpu, u32MaxThreadID, sRdPkgConfigRes.u8CompletionCode);
#ifdef BUILD_RAW
				fwrite(&u32MaxThreadID, sizeof(u32MaxThreadID), 1, fpRaw);
#endif //BUILD_RAW
			} else {
				// skip to the next CPU if not 0x40/0x91/0x94.
				PRINT(PRINT_DBG, PRINT_ERROR, "[DBG] - Failed to get number of threads for CPU #%d (cc=0x%x)\n", u8Cpu, sRdPkgConfigRes.u8CompletionCode);
				continue;
			}
		}
        ...
        for (u32CoreNum = 0; u32CoreNum <= u32MaxThreadID / 2; u32CoreNum++) {
            ...
            for (j = FIRST_CORE_MCA; j <= LAST_CORE_MCA; j++) {
				...
				for (u8Dword = 0; u8Dword < CM_NUM_MCA_DWORDS; u8Dword++) {
					...
					if (u8Dword % 2 == 0)
						sCoreMcaRawData[j].u8CC[u8Dword / 2] = PECI_CC_SUCCESS;
					...
					if (ePECIStatus != PECI_CC_SUCCESS) {
						...
						sCoreMcaRawData[j].u8CC[u8Dword / 2] = sRdPkgConfigRes.u8CompletionCode;

						// MCE Bank sequence failed, abort the sequence on non-0x40/0x91/0x94 and go to the next Bank
						if (ePECIStatus != PECI_CC_MAYBE_SUCCESS && ePECIStatus != PECI_CC_CMD_PASSED) {
							...
							PECI_WrPkgConfig(&sWrPkgConfigReq, &sWrPkgConfigRes);
							PRINT(PRINT_DBG, PRINT_ERROR, "[DBG] - CPU #%d Core %ld MCE Bank %ld Sequence Failed\n", u8Cpu, u32CoreNum, j);
							sCoreMcaRawData[j].uRegData.u32Raw[0] = ePECIStatus;
