...
mctppcie_hw_init(void)
{
	...
	IRQ_NUMBER = MCTP_IRQ;
#endif
	mctp_irq = GetIrqFromDT("ami_mctppcie", IRQ_NUMBER);

	
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5,2,0))
	if ((ret = request_irq(mctp_irq, mctppcie_handler, IRQF_DISABLED, MCTP_PCIE_DEV_NAME, 0)) < 0)
#else
  if ((ret = request_irq(mctp_irq, mctppcie_handler, 0, MCTP_PCIE_DEV_NAME, 0)) < 0)
#endif
	{
		printk("ERROR: Failed to request PECI irq %d name %s (err %d)",mctp_irq,MCTP_PCIE_DEV_NAME, ret);
		ret = -EIO;
		goto fail;
	}
