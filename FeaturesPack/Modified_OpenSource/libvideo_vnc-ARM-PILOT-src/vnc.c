...
//	u32 data_buf_size=0, frame_hdr_size=0;
	...
	if((frame_hdr_vnc.res_x >= VNC_MIN_RESX) && (frame_hdr_vnc.res_y >= VNC_MIN_RESY)  && (capture_status != VIDEO_NO_SIGNAL))
	{
		if ((frame_hdr_vnc.res_x != maxx) || (frame_hdr_vnc.res_y != maxy))
		{
			...
			{
				ioctl(fd, AMIFB_GET_MODEDESC, &md);
				close(fd);
			}

			if (md.dwPitch != 0) /* ioctl success - use fb_vnc data */
			{
				/* Ref: Pilot4 RVAS Document > 1.2.3 Graphics Display Controller in non-VGA Mode */
				if (((unsigned)maxx != md.dwPitch) || ((unsigned)maxy != md.dwHeight))
				{
					maxx = frame_hdr_vnc.res_x = md.dwPitch;
					maxy = frame_hdr_vnc.res_y = md.dwHeight;
					return RESOLUTION_CHANGED;
				}
			}
			else /* ioctl failure - use videocap data */
			{
