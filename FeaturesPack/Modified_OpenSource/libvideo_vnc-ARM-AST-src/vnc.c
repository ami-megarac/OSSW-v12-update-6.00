...
int startCapture()
{
	...
	{
		frame_hdr_vnc.res_x = ((frame_hdr_t *)p_frame_hdr)->SourceMode_X;
		frame_hdr_vnc.res_y = ((frame_hdr_t *)p_frame_hdr)->SourceMode_Y;
		frame_hdr_vnc.params.video_mode_flags = 0; //text mode not implemented
		frame_hdr_vnc.frame_addr = p_data_buf;
		frame_hdr_vnc.params.BytesPerPixel = (u8)(((frame_hdr_t *)p_frame_hdr)->FrameHdr_Mode420 != 0 ? BLOCK_SIZE_YUV420 : BLOCK_SIZE_YUV444);/* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive */ /*BytesPerPixel unsigned short int and will be assigned with BLOCK_SIZE_YUV420 (0x10) BLOCK_SIZE_YUV420 (0x10) which are in a limited range - Reason*/
