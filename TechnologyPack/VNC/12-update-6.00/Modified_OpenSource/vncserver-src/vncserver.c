...
static void clientgone(rfbClientPtr cl) {
	...
	if(active_sock_list == NULL){
	 	active_sock_list = (SOCKET *)malloc((sizeof(SOCKET) * actvSessCount));/* Fortify [Integer Overflow]:: False Positive */ /* condition to check integer overflow of actvSessCount is present - Reason*/
	 }
	...
	if(actvSessCount == 0)
	{
		memset((char *)sessionInfo, 0, (sizeof(VNCSessionInfo) * vnc_max_session)); /* Fortify [Buffer Overflow]:: False Positive */ /* The number of bytes set to the null will be in limited range - Reason*/
...
#endif
	emptyScreen= (char*)malloc(maxx*maxy*bpp);/* Fortify [Integer Overflow]:: False Positive */ /* condition to check integer overflow of maxa and maxy is present - Reason*/
...
void adoptResolutionChange(rfbClientPtr cl) {
	...
	iterator = rfbGetClientIterator(cl->screen);/* Fortify [Memory Leak]:: False Positive */ /*Allocated memory is released upon calling rfbReleaseClientIterator() - Reason*/
		...
		while(1){
			...
			if(powerCons == 1){
				...
				if(RESOLUTION_CHANGED == capture){
					...
					#elif defined(SOC_AST2400) || defined(SOC_AST2500)
							posx = (x-1) *TILE_WIDTH;
							width = TILE_WIDTH;
							height = TILE_HEIGHT;
					#endif
...
void showBlankScreen(char *message,int xPos,int yPos) {
	...
	memset(emptyScreen, 0, emptyScreenSize); /* Fortify [Buffer Overflow]:: False Positive */ /* The number of bytes set to the null is in limited range - Reason*/
...
void cleanUpInvalidSession(int sessionIndex, int reason)
{
	...
	sessionInfo[sessionIndex].status = UNUSED; /* Fortify [Buffer Overflow]:: False Positive */ /*status is unsigned short int and will be assigned with UNUSED(0) which is in a limited range - Reason */
...
{
	sessionInfo = (VNCSessionInfo *) malloc ((sizeof(VNCSessionInfo) * vnc_max_session));// Fix for Fortify [Missing Check against Null] 
	memset((char *)sessionInfo, 0, (sizeof(VNCSessionInfo) * vnc_max_session));/* Fortify [Intger Overflow]:: False Positive */ /* vnc_max_session is depending on vnc_max count which is in limited range - Reason */
	return sessionInfo;
}
pthread_mutex_t  *getVncSessionGuard()
{
	vnc_session_guard = (pthread_mutex_t *)calloc(vnc_max_session, sizeof(pthread_mutex_t));/* Fortify [Intger Overflow]:: False Positive */ /* vnc_max_session is depending on vnc_max count which is in limited range - Reason */
...
int openVncPipe(char *pipename) {
	...
	if (pipename != NULL) {
		...
		if (fd < 0) {
			...
			if(unlink(pipename) == -1)/* Fortify [Race Condition: File System Access]:: False Positive */ /* The file does not change between the calls of unlink() and open() - Reason*/
				printf("[%d]:could not unlink the pipe %s\n", errno, pipename);
		}
	}
	return fd;
		...
		if (read(vncpipe, &cmd, sizeof(cmd)) > 0) {
			...
			switch (cmd.cmd)
			{
				...
				case RECV_VNC_CONF:
					...
					{
						vncConfUpdate = 1;
						if(read(vncpipe, &vncConf, cmd.datalen) > 0) /* Fortify [Buffer Overflow]:: False Positive */ /* The number of bytes attempt to be read will be limited to cmd.datalen - Reason*/
						{
							...
							//close the active VNC clients
							rfbShutdownServer(rfbScreen,1);
							if(rfbScreen)
								rfbScreenCleanup(rfbScreen);
							rfbScreen = NULL;
							//Reinitialize vnc server
				...
				case DISCONNECT_VNC_SESSION:
					...
					if(read(vncpipe, &vnc_session, sizeof(vnc_clean_session_t)) > 0) /* Fortify [Buffer Overflow]:: False Positive */ /* The number of bytes attempt to be read will be limited to sizeof(vnc_clean_session_t) - Reason*/
					{
						sessionInfo[ vnc_session.session_index ].status = vnc_session.reason;/* Fortify [Buffer Overflow]:: False Positive */ /*vnc_session.reason is depending on status which is 0 or 1  it in limited range - Reason */ 
					}
					else
					{
...
int WriteCmdInAMIVncPipe(int cmd, int status){
	    ...
	    if(AmiVNCPipe != -1)
	    {
	        comm_info.cmd = (unsigned short) cmd;/* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive */ /* Reason for False Positive - Casting for fortify [Type Mismatch: Signed to Unsigned].*/
	        comm_info.datalen = sizeof(cmd_info_t);
	        comm_info.status = (unsigned short) status;/* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive */ /* Reason for False Positive - Casting for fortify [Type Mismatch: Signed to Unsigned].*/
...
#ifndef CONFIG_SPX_FEATURE_DISABLE_KVM_AUTO_SWITCH
...
int send_session_info( int index) {
	...
	if(AmiVNCPipe != -1)
	{
		...
		comm_info.cmd = SEND_SESSION_INFO;/* Fortify [Buffer Overflow]:: False Positive */ /*cmd is unsigned short int and will be assigned with SEND_SESSION_INFO(0x08) which is in a limited range - Reason */
		comm_info.datalen = sizeof(VNCSessionInfo);
		comm_info.status = (unsigned short) index; /* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive */ /* Reason for False Positive - Casting for fortify [Type Mismatch: Signed to Unsigned].*/
		if(write(AmiVNCPipe,&comm_info,sizeof(cmd_info_t)) == -1)
		{
			printf("Unable to write into %s pipe\n",VNC_AMI_CMD_PIPE);
			return -1;
...
{
	char *lo = "127.0.0.1", *lo6 = "::1";
	struct stat statbuff;

	if (cl->host != NULL)
	{
		if (!((strncmp(cl->host, lo, strlen(cl->host)) == 0) || (strncmp(cl->host, lo6, strlen(cl->host)) == 0)))
		{
