...
CrashdumpSection sectionNames[NUMBER_OF_SECTIONS] = {
    {"MCA", UNCORE_MCA, record_type::mcaLog, logUncoreMca
#ifdef SPX_BMC_ACD
    , EN_DATA_AREA
#endif
},
    ...
    {"MCA", MCA, record_type::mcaLog, logCoreMca
#ifdef SPX_BMC_ACD
    , EN_DATA_AREA
#endif
},
...
{
    cJSON* logSection = NULL;
    if((sectionName.flag & EN_DATA_AREA) == DIS_DATA_AREA)
        return ;
...
{
    cJSON* logSection = NULL;
    if((sectionNames[Section::METADATA].flag & EN_DATA_AREA) == DIS_DATA_AREA)
        return ;
                     ...
                     bool isTelemetry)
{
    ...
    if ((crashdump::cpu::icx == cpuInfo[0].model) ||
        ...
        (crashdump::cpu::clx == cpuInfo[0].model) ||
        (crashdump::cpu::cpx == cpuInfo[0].model) ||
        (crashdump::cpu::skx == cpuInfo[0].model))
    {
...
#ifdef SPX_BMC_ACD
static int IsValidError(std::vector<crashdump::CPUInfo>& cpuInfo)
{
    ...
    if(cpuInfo.size() == 0)
    {
        crashdump::initCPUInfo(cpuInfo);
    
        crashdump::getCPUData(cpuInfo, cpuid::STARTUP);
...
int main(int argc, char* argv[])
{
    ...
    int validTrigger = 0, checkErrCount = 0, ret = 0;
...
/*Getting Max CPUS*/
    ...
    RecordDataAreaConfig((void *)crashdump::sectionNames);
    while(1) 
    {
        ret = IsACDTriggered();
        if(ret == 2)
        {
            DataAreaConfig((void *)crashdump::sectionNames);
            continue;
        }
        else if ( ret != 1)
        {
        ...
        validTrigger = 1;
        checkErrCount = 0;
        while (!IsValidError(crashdump::cpuInfo))
        {
