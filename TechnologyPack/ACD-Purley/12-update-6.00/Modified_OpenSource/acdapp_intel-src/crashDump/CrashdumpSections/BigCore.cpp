...
static SCrashdumpRegICX1 sCrashdumpCoreHeaderICX1[] = {
    // Register-name, Size
    {"crashlog_version", 4, false},     {"size", 4, false},
    {"art_ctc_timestamp", 8, false},    {"patch_revid", 4, false},
    {"shutdown_reason", 4, false},      {"whoami", 4, false},
...
static SCrashdumpRegICX1 sCrashdumpCoreRegsICX1[] = {
    ...
    {"SCP_CR_THD_SPECIFIC_PATCHING_2", 8, false},
    {"SCP_CR_POST_COUNTER", 4, false},
    {"SCP_CR_FLOW_DETAILS", 4, false},
    {"SCP_CR_THD_SPECIFIC_PATCHING_0", 4, false},
    {"SCP_CR_THD_SPECIFIC_PATCHING_1", 4, false},
    {"SCP_CR_MCG_STATUS", 4, true},
    {"SCP_CR_PSMI_CTRL", 4, false},
    {"SCP_CR_TARGET_SLEEP_STATE", 4, false},
    {"SCP_CR_PATCH_REV_ID", 4, false},
    {"DebugCTLMSR", 8, false},
    {"LER_FROM", 8, false},
    {"LER_TO", 8, false},
    {"LER_INFO", 8, false},
...
acdStatus storeBigCoreHeader(const bool limitedOutput, uint32_t* u32DataIndex,
                             const uint32_t u32NumReads, const int retval,
                             const uint8_t cc, uint8_t* pu8Crashdump,
                             uint32_t u32CrashSize, cJSON** ppthread)
{
    // Add the header data
    for (size_t i = 0;
         ...
         i++)
    {
        ...
        if (isRegisterLogged(limitedOutput,
                             sCrashdumpCoreHeaderICX1[i].isLogged))
        {
            if (u32NumReads < (*u32DataIndex))
            {
                if (retval != PECI_CC_SUCCESS)
                {
                    ...
                    cJSON_AddStringToObject(*ppthread,
                                            sCrashdumpCoreHeaderICX1[i].name,
                                            jsonItemString);
                    return ACD_FAILURE;
                }
                else if (PECI_CC_UA(cc))
            ...
            cJSON_AddStringToObject(*ppthread, sCrashdumpCoreHeaderICX1[i].name,
                                    jsonItemString);
        }
    }
...
acdStatus storeBigCoreRegister(const bool limitedOutput, uint32_t* u32DataIndex,
                               const uint32_t u32NumReads, const int retval,
                               const uint8_t cc, uint8_t* pu8Crashdump,
                               uint32_t u32CrashSize, cJSON** ppthread)
{
         ...
         i < (sizeof(sCrashdumpCoreRegsICX1) / sizeof(SCrashdumpRegICX1)); i++)
    {
        ...
        if (isRegisterLogged(limitedOutput, sCrashdumpCoreRegsICX1[i].isLogged))
        {
            if (u32NumReads < (*u32DataIndex))
            {
                if (retval != PECI_CC_SUCCESS)
            ...
            cJSON_AddStringToObject(*ppthread, sCrashdumpCoreRegsICX1[i].name,
                                    jsonItemString);
        }
    }
...
acdStatus storeBigCoreLBR(const bool limitedOutput, uint32_t* u32DataIndex,
                          const uint32_t u32NumReads, const int retval,
                          const uint8_t cc, uint8_t* pu8Crashdump,
                          uint32_t u32CrashSize, cJSON** ppthread)
{
            ...
            if (!limitedOutput)
            {
                ...
                if (u32NumReads < (*u32DataIndex))
                {
                    if (retval != PECI_CC_SUCCESS)
                    {
                        cd_snprintf_s(jsonItemString, CD_JSON_STRING_LEN,
                                      CD_JSON_UA_DF, cc, retval);
                        cJSON_AddStringToObject(*ppthread, jsonItemName,
                                                jsonItemString);
                        return ACD_FAILURE;
                    }
                    else if (PECI_CC_UA(cc))
...
acdStatus storeBigCoreSqData(uint32_t* u32DataIndex, const uint32_t u32NumReads,
                             const int retval, const uint8_t cc,
                             uint8_t* pu8Crashdump, uint32_t u32CrashSize,
                             cJSON** ppcore, cJSON** ppthread)
{
    // if there is still data, it's the SQ data, so dump it
    // Add the SQ Dump item to the Crashdump JSON structure
    cJSON* sqDump;
    cJSON_AddItemToObject(*ppcore, "SQ", sqDump = cJSON_CreateObject());
...
static void crashdumpJsonICX1(uint32_t u32CoreNum, uint32_t u32ThreadNum,
                              ...
                              uint8_t cc, int retval, bool isLimitedOutput)
{
    char jsonItemName[CD_JSON_STRING_LEN] = {0};
    ...
    acdStatus error =
        storeBigCoreHeader(isLimitedOutput, &u32DataIndex, u32NumReads, retval,
                           cc, pu8Crashdump, u32CrashSize, &thread);
    if (error)
    {
        return;
    }
    ...
    error =
        storeBigCoreRegister(isLimitedOutput, &u32DataIndex, u32NumReads,
                             retval, cc, pu8Crashdump, u32CrashSize, &thread);
    if (error)
    {
        return;
    }

    error = storeBigCoreLBR(isLimitedOutput, &u32DataIndex, u32NumReads, retval,
                            cc, pu8Crashdump, u32CrashSize, &thread);
    if (error)
    {
        return;
    ...
    if (!isLimitedOutput)
    {
        error = storeBigCoreSqData(&u32DataIndex, u32NumReads, retval, cc,
                                   pu8Crashdump, u32CrashSize, &core, &thread);
        if (error)
        {
            return;
        }
...
int logCrashdumpICX1(crashdump::CPUInfo& cpuInfo, cJSON* pJsonChild)
{
            ...
            "_use_reg_enable"))
    {
        ...
        CRASHDUMP_PRINT(
            ERR, stderr,
            "\"_use_reg_enable\" is set to true. Limiting output for "
            "Big Core section\n");
    }
    else
    {
                 ...
                 u32ThreadNum++)
            {
                ...
                else
                {
                    crashdumpJsonICX1(u32CoreNum, threadId, u32CrashdumpSize,
                                      ...
                                      pJsonChild, cc, ret, isLimitedOutput);
                }
                FREE(pu64Crashdump);
