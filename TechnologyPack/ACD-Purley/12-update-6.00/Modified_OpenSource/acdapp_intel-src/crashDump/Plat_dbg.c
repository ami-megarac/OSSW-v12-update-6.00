...
bool ACD_ON_DEMAND_Request = FALSE;
...
#define ACD_BAFI_LIB "/usr/local/lib/libacdbafi.so"
...
static pthread_t *sg_MonitorThread = NULL;
...
#endif
{
    ...
    switch (pSPlatDBG->ePlatDBGCmd) {
...
#ifdef BUILD_RAW
#ifndef SPX_BMC_ACD
            sprintf(rawFilename, "%s/%s", CRASHDUMP_DIRECTORY, RAM_CPUDUMP_RAW_FILE);
#else
            sprintf(rawFilename, "%s/%s", crashdump_dir, RAM_CPUDUMP_RAW_FILE);
#endif
            fpRaw = fopen(rawFilename, "wb");
#endif //BUILD_RAW

            // Loop through the list of log sections and generate each one
            if (TRUE == ACD_ON_DEMAND_Request)
            {
                debugLogCount =(sizeof(sDebugLogTable) / sizeof(SDebugLogSection)) - 4;
            } else {
                debugLogCount =(sizeof(sDebugLogTable) / sizeof(SDebugLogSection));
            }
#ifndef SPX_BMC_ACD
	    ...
	    for (i = 0; i < debugLogCount; i++) {
                ...
                printf("Logging %s : %s", sDebugLogTable[i].sectionName, tempr);
                fflush(stdout);
#endif //TIME_SECTION
                ...
                memset_s(tempr, sizeof(tempr), 0);
                GetClockTime(tempr, 63);
                printf("Finished logging %s : %s", sDebugLogTable[i].sectionName, tempr);
                fflush(stdout);
#endif //TIME_SECTION
            }
...
{
	struct flock fl;
	int fd, dirLen = strlen(crashdump_json_dir), fileLen = strlen(ACD_INSTANCE_LOCK);

	//snprintf(ooi_path, sizeof(ooi_path)-1, "%s%s", crashdump_json_dir, ACD_INSTANCE_LOCK);
    memcpy(ooi_path, crashdump_json_dir, dirLen);
    memcpy(&ooi_path[dirLen], ACD_INSTANCE_LOCK, fileLen);
...
int main (int argc, char **argv)
{
	...
	char crashdump_dir[DIR_NAME_LEN] = {0};
	char crashdump_json_dir[DIR_NAME_LEN] = {0};
#endif
	...
	memcpy(crashdump_dir, CRASHDUMP_DIRECTORY, strlen(CRASHDUMP_DIRECTORY));
	memcpy(crashdump_json_dir, CRASHDUMP_JSON_DIRECTORY, strlen(CRASHDUMP_JSON_DIRECTORY));
...
#if 0//def CONFIG_SPX_FEATURE_ACD_BAFI_SUPPORT
    int (*BAFI_Decoder)() = NULL;
    dl_handle_bafi = dlopen((char *)ACD_BAFI_LIB, RTLD_NOW | RTLD_GLOBAL | RTLD_NODELETE);
	if(!dl_handle_bafi)
	{
		printf("Error in loading ACD_BAFI_LIB library %s\n",dlerror());
        dlclose(dl_handle);
		return 1;
	}

    BAFI_Decoder = dlsym(dl_handle_bafi, "BAFI_Decoder");
    if (BAFI_Decoder == NULL)
    {
        dlclose(dl_handle);

        if(dl_handle_bafi != NULL)
            dlclose(dl_handle_bafi);
...
#if 0//def CONFIG_SPX_FEATURE_ACD_BAFI_SUPPORT
        /* BAFI decoder need the data from section "TOR_dump" and "crashdump" at the same time.
         * However, when we use the Crashdump application manually with parameter "--dump-section <section num>",
         * we can choose only one section.
         * In this case, BAFI decoder will not support. */
        if (saveArg == -1)
        {
            if (BAFI_Decoder())
                printf("Tordump decoder failed\n");
        }
#endif
...
/*        if( access( "/var/crashdump/json/sysdebug2.json", F_OK ) != -1 ) {
 ...
 /*       if( access( "/var/crashdump/json/sysdebug2.json", F_OK ) != -1 ) {
        ...
        // create BAFI.json using acdbafigenerator binary app
        system("/usr/local/bin/acdbafigenerator -t /var/crashdump/json/sysdebug.json");
#endif
        system("/etc/init.d/crashdump_compress.sh &");
