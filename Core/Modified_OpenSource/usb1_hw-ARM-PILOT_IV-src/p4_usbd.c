...
static int  PHYS_P4_USBD_IRQ[] = {PHYS_P4_USBD_IRQ_HUB,
							  PHYS_P4_USBD_IRQ_DEV0,
							  PHYS_P4_USBD_IRQ_DEV1,
							  PHYS_P4_USBD_IRQ_DEV2,
							  PHYS_P4_USBD_IRQ_DEV3,
							  PHYS_P4_USBD_IRQ_DEV4,
							  PHYS_P4_USBD_IRQ_DEV5,
							  PHYS_P4_USBD_IRQ_DEV6,
};

static int mapped_irq[P4_USBD_MAX_DEV_PORT];
...
static DEFINE_SPINLOCK(g_Hub_Lock);
#else
static spinlock_t g_Hub_Lock = SPIN_LOCK_UNLOCKED;
#endif
#define HUB_SPINLOCK_IRQSAVE(flags)	spin_lock_irqsave(&g_Hub_Lock, flags)
#define HUB_SPINUNLOCK_IRQRESTORE(flags)	spin_unlock_irqrestore(&g_Hub_Lock, flags)
...
static void p4_usbd_hub_send_port_change(struct work_struct *qworker_data)
{
	...
	kfree( (void *)qworker_data ); // free at first to reduce the function calls and avoid memory leak if early return.
	
	while( (p4_usbd_read8_reg(P4_USBD_INxCS[1]) & P4_USBD_BIT1_SET) ) {
		if (P4_USBD_HUB_CONFIG_VALUE != p4_usbd_hub_cfg_current_config_value)
		{	// Discard the retry of send port change if USB hub is not configured.
			return ;
		}
		mdelay(5);
		if(retry > 250) {
	...
	if (P4_USBD_HUB_CONFIG_VALUE != p4_usbd_hub_cfg_current_config_value)
	{	// may the reset USB interface happened...discard the send port change.
		return ;
	}
...
static uint8_t p4_usbd_hw_init(void)
{
	...
	if( !p4_usbd_get_power_status() ) {printk(KERN_ERR "P4_USBD(%d): Can't init usb hw\n", __LINE__);
		return 0;
	}
...
static int  p4_usbd_SWC_interrupt_cb(void)
{
	...
	if( p4_usbd_get_power_status() ) {
		timeout = 0;
		while(timeout < 500)
		{	// wait for a while for stable power good signal.
			udelay(1000);
			timeout += 1;
		}
		...
		// do all init needed, cause of reset USB interface that clear all register value.
		// enable hub, enable ep, enable interrupt, setup DMA buffer address, set DMA transfer length.
		p4_usbd_state_init(0);
		...
		for ( devNum = 0; devNum < P4_USBD_MAX_DEV_NUM; devNum++ ) {
			if(p4_usbd_device_hwEnabled[devNum] == 1) {
				...
				p4_usbd_port_enable(devNum);
				usb_core_module.CoreUsbDeviceReconnect(devNum);
				usb_core_module.CoreUsbEnableAllEndPoints(devNum);
			}
		}
	} else {
		// host power off.
		
		/* Disable the top level USB Interrupt */
		// Disable HUB. disconnect the HUB.
		p4_usbd_write8_reg(p4_usbd_read8_reg(P4_USBD_BMCINTEN) & (~(1 << 0)), P4_USBD_BMCINTEN);
		...
		for ( devNum = 0; devNum < P4_USBD_MAX_DEV_NUM; devNum++ ) {
			if(p4_usbd_device_hwEnabled[devNum] == 1) {
				...
				// discard ongoing and incoming transfers when host is powered off.
				HUB_SPINLOCK_IRQSAVE(lock_flag);
				usb_core_module.CoreUsbBusReset(devNum);
				HUB_SPINUNLOCK_IRQRESTORE(lock_flag);
				mdelay(5); // wait a while for incoming response from client.
			}
			
			if (1 == p4_usbd_device_connect_to_port[devNum])
			{ // Make the devices connected during host on after resetting USB interface.
				p4_usbd_device_connect_to_port[devNum] = 0;
			}
			mdelay(2); // wait a while for incoming response from client.
			
		}
		
		// reseting USB interface to avoid B4 issue with DMA mode.
		p4_usbd_write8_reg(0x0F, P4_USBD_BMCINTEN); // No matter what the value. Just for checking if the reset has actually finished
		*(volatile uint32_t*)IO_ADDRESS(SE_LPC_ROM_VA_BASE + 0x854) = 0x20; // Reset USB Interface
		*(volatile uint32_t*)IO_ADDRESS(SE_LPC_ROM_VA_BASE + 0x858) = 0x00; // Make sure we dont reset other modules
		*(volatile uint32_t*)IO_ADDRESS(SE_LPC_ROM_VA_BASE + 0x850) = 0xD01;
		
		timeout = 0;
		flags = p4_usbd_read8_reg(P4_USBD_BMCINTEN);
		while(flags !=0 && timeout < 50) {
			flags = p4_usbd_read8_reg(P4_USBD_BMCINTEN);
			udelay(100);
			timeout++;
		}
...
static int __init p4_usbd_probe(struct platform_device *pdev)
{
	...
	p4_usbd_dv_wq = alloc_workqueue("p4_usbd_dv_wq", WQ_HIGHPRI, 1);
 	...
 	GetMultipleIrqFromDT("ami_usbhub", P4_USBD_MAX_DEV_PORT, PHYS_P4_USBD_IRQ, mapped_irq);


	status = request_irq(mapped_irq[0], &p4_usbd_interrupt, IRQF_SHARED, p4_usbd_driver_name, pdev);
	if(status < 0) {
	...
	for(i = 1; i < P4_USBD_MAX_DEV_PORT; i++) {
		status = request_irq(mapped_irq[i], &p4_usbd_interrupt_dev, IRQF_SHARED, p4_usbd_driver_name, pdev);
		if(status < 0) {
			printk(KERN_ERR "P4_USBD(%d): request_irq failed for dev %d\n", __LINE__, i);
			goto cleanup;
		}
...
static int p4_usbd_hw_alloc_buffer (uint8_t devNum, uint8_t epNum, uint16_t fifoSize, uint16_t maxPkt, uint8_t dir, uint8_t epType)
{
	...
	if( dir == DIR_IN ) {
		p4_usbd_dma_ptr = &p4_usbd_dma[ 1 + epNum ];
		// DMA memory allocated once only to avoid memory leak.
		if (NULL == p4_usbd_dma_ptr->in_virt_addr) {
		...
		// reset whole buf with allocated size P4_USBD_MAX_DMA_PKTS.
		p4_usbd_dma_virt = (uint8_t*)p4_usbd_dma_ptr->in_virt_addr;
		memset(&p4_usbd_dma_virt[0], 0x00, P4_USBD_MAX_DMA_PKTS);
    } else {
		p4_usbd_dma_ptr = &p4_usbd_dma[ 1 + P4_USBD_MAX_IN_EP + epNum ];
		// DMA memory allocated once only to avoid memory leak.
		if (NULL == p4_usbd_dma_ptr->out_virt_addr) {
...
{
	if( epNum == 0 ) {
		*stall = ( p4_usbd_read8_reg(P4_USBD_DEVXEP0CTRL(devNum)) & P4_USBD_EP0CS_STALL )?1:0;
	} else {
		if(epDir) {
...
static int p4_usbd_hw_stall_ep (uint8_t devNum, uint8_t epNum, uint8_t epDir)
{
	//printk("stall %d,%d,%d\n", devNum, epNum, epDir);
	if ( epNum == 0 ) {
		p4_usbd_write8_reg((p4_usbd_read8_reg(P4_USBD_DEVXEP0CTRL(devNum)) & ~P4_USBD_EP0CS_HSNAK) | P4_USBD_EP0CS_STALL , P4_USBD_DEVXEP0CTRL(devNum));
	} else {
		if( epDir ) {
...
static void p4_usbd_hw_device_disconnect (uint8_t devNo)
{
	...
	/* Ref: Pilot4_SDK_document_1_06 > 3.1.12.1.4 Device Disconnect
	 * Need to set disconnect bit in DEVXDISCONDB register to indicate HUB that
	 * the device is removed. Update BMCINTEN, DEVxEP0INTEN, DEVX2HUBINTEN only
	 * after updating DEVXDISCONDB. */
	p4_usbd_write8_reg(1, P4_USBD_DEVXDISCONDB(devNo));
	
	HUB_SPINLOCK_IRQSAVE(lock_flag);
	if (P4_USBD_HUB_CONFIG_VALUE == p4_usbd_hub_cfg_current_config_value)
	{	// only avavaile when HUB is configured during host enumeration.
...
static uint8_t p4_usbd_state_init(uint8_t isHWModuleLoading)
{
	...
	if(isHWModuleLoading) {
		...
		// A2
		p4_usbd_sedid = (*(volatile uint32_t*)IO_ADDRESS(SE_SYS_CLK_BASE+0x50)) & 0xFF;
		if( p4_usbd_sedid == 0x42 ) {
			p4_usbd_p2connect= *(volatile uint32_t*)IO_ADDRESS(SE_FGE_BASE+0x74);
...
#if P4_USBD_DMA_SUPPORT
static void p4_usbd_dma_init(void)
{
	memset(p4_usbd_dma, 0x00, sizeof(p4_usbd_dma));
}

static void p4_usbd_dma_cleanup(void)
{
	int i = 0;
	int num = (sizeof(p4_usbd_dma)/sizeof(p4_usbd_dma[0]));
	struct p4_usbd_dma_t* p4_usbd_dma_ptr = NULL;
	for(i = 0; i < num; i++)
	{
		p4_usbd_dma_ptr = &p4_usbd_dma[i];
		if(!p4_usbd_dma_ptr->isDma)
			continue;
		p4_usbd_dma_ptr->isDma = 0;
		// release in endpoint dma memory if allocated.
		if(NULL != p4_usbd_dma_ptr->in_virt_addr)
		{
			dma_free_coherent(NULL, P4_USBD_MAX_DMA_PKTS, p4_usbd_dma_ptr->in_virt_addr, p4_usbd_dma_ptr->in_phy_addr);
			p4_usbd_dma_ptr->in_virt_addr = NULL;
		}
		// release out endpoint dma memory if allocated.
		if(NULL != p4_usbd_dma_ptr->out_virt_addr)
		{
			dma_free_coherent(NULL, P4_USBD_MAX_DMA_PKTS, p4_usbd_dma_ptr->out_virt_addr, p4_usbd_dma_ptr->out_phy_addr);
			p4_usbd_dma_ptr->out_virt_addr = NULL;
		}
	}
}
#endif
