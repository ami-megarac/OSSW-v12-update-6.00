	...
	for (IfaceNum=0;IfaceNum<bNumInterfaces;IfaceNum++)
	{
		...
		for (EpNum = 1; ((EpNum <= TotalEndPoints) && bNumEndPoints); EpNum++)
		{
			for (Direction = DIR_OUT; Direction <= DIR_IN; Direction++)
			{
				...
				{
					bNumEndPoints -= 1;
					EpDir = (EndDesc->bEndpointAddress & IN)?DIR_IN:DIR_OUT; /* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive *//* Reason for False Positive - Conditional operator return 0 or 1, no any negative values. */
...
UsbRestoreInitState(uint8 DevNo)
{
	...
	if (MAX_USB_HW <= DevNo) /* Fix for Fortify [Out-of-Bounds Read: Off-by-One].*/ // Valid range: 0 ~ (MAX_USB_HW-1).
	{
	...
	DevInfo[DevNo].InRequest.DataLen = 0; /* Fortify [Out-of-Bounds Read: Off-by-One]:: False Positive */
	DevInfo[DevNo].InRequest.wLength = 0; /* Fortify [Out-of-Bounds Read: Off-by-One]:: False Positive */

	DevInfo[DevNo].OutRequest.DataLen = 0; /* Fortify [Out-of-Bounds Read: Off-by-One]:: False Positive */
	DevInfo[DevNo].OutRequest.wLength = 0; /* Fortify [Out-of-Bounds Read: Off-by-One]:: False Positive */
	...
	{
		DevInfo[DevNo].EndPoint[EpIndex].TxReady = 1; /* Fortify [Out-of-Bounds Read: Off-by-One]:: False Positive */
		DevInfo[DevNo].EndPoint[EpIndex].TxData = NULL; /* Fortify [Out-of-Bounds Read: Off-by-One]:: False Positive */
	}
...
/*	EpInfo->Enabled 	  	= 1; */
	...
	else
	{
		...
		EpInfo->Dir				= (EndDesc->bEndpointAddress & 0x80)?DIR_IN: DIR_OUT; /* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive *//* Reason for False Positive - Conditional operator return 0 or 1, no any negative values. */
	}
...
UsbWriteDataNonBlock(uint8 DevNo, uint8 ep, uint8 *Buffer, uint32 Len, uint8 ZeroLenPkt)
{
	...
	if ((EpInfo->TxData != NULL)|| (EpInfo->TxReady != 1))
	{
		TDBG("UsbWriteDataNonBlock():Previous Write Pending for Dev %d Ep %d\n",DevNo,ep);
		TDBG("UsbWriteDataNonBlock():TxData=0x%lx DataLen=0x%x DataPos=0x%x Zero=%d Ready=%d\n",
		       (unsigned long)EpInfo->TxData,
		       EpInfo->TxDataLen,EpInfo->TxDataPos,
		       EpInfo->TxZeroLenPkt,EpInfo->TxReady);
	}
...
UsbUpstreamDisable_Device_all ()
{
	...
	for (DevNo = 0; DevNo < MAX_USB_HW; DevNo++) {
		...
		/**
		 * Clean up for eth usb0 issue.
		 * SYMPTOM:
		 * Continually driver message: "Previous Write Pending for Dev 2 Ep 3" by UsbWriteDataNonBlock().
		 * 
		 * ROOT CAUSE:
		 * 
		 * When usb0 is up, kernel send network packet via usb0 continually(known as neigh_hh_output() and neigh_resolve_output()).
		 * The USB transfer is not completed with ongoing network packet, once disable upstream via power save mode or otherwise.
		 * It makes TxData as null, but TxReady is not 1 (should called by ast_usbhub_ep_tx_done()).
		 * In this case, when next network packet coming, driver triggers the message above.
		 * 
		 * RESOLUTION:
		 * Avoid to the USB transfer for incoming network packet when upstream disabled.
		 * Due to it is not able to stop kernel sending network packet.
		 * Add the Cleanup to reset the configuration of usb0 as dev 2 and make the network packet sending flow stopped early by EthNetTxHandler().
		 */
		if(2 == DevNo)
		{
			UsbRestoreInitState(DevNo);
