...
static int irq_icmb;
static int mapped_irq[BUS_COUNT];
...
static int monitor_seq_show(struct seq_file *seq_file, void *v)
{
	...
	if ((unsigned int)(*spos) == int_monitor.monitor_count)
		...
		retval += 
#endif
		seq_printf (seq_file, "S.No  Jiffies          Bus  Raw  Served  Status  TX_abrt_src  TXFLR  RXFLR  Snd/Rcv    Data[]\n");
	...
	retval += 
#endif
	seq_printf (seq_file, "%4i  %15llu  %03X  %3X  %6X  %6X  %11X  %5X  %5X  %7X    ", 
	...
	for (j = 0; j < int_monitor.int_data[count].data_count; j++)
	{
		...
		retval += 
#endif
		seq_printf (seq_file, " %2X", int_monitor.int_data[count].data_bytes[j]);
	}
...
static int bus_busy_seq_show(struct seq_file *seq_file, void *v)
{
	...
	if ((unsigned int)(*spos) == bus_busy_monitor.monitor_count)
		...
		retval += 
#endif
		seq_printf (seq_file, "S.No  Jiffies          Bus  CON  TAR  SAR  INTR_STAT  RX_TL  TX_TL  ENABLE  STATUS  TXFLR  RXFLR  TX_ABT_SRC  DEBUG_STS\n");
	...
	retval += 
#endif
	seq_printf (seq_file, "%4i  %15llu  %03X  %3X  %3X  %3X  %9X  %5X  %5X  %6X  %6X  %5X  %5X  %10X  %9X\n", 
...
i2c_pilot_ii_init( void )
{
  ...
  GetMultipleIrqFromDT("ami_i2c", bus_count, phys_irq, mapped_irq);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,2,0))
   if( request_irq( mapped_irq[ i ], irq_handlers[i], 0, "i2c_pilot_ii", (void *)irq_vadd[i]) < 0 )
#else
#if (LINUX_VERSION_CODE >=  KERNEL_VERSION(2,6,24)) 
         if( request_irq( mapped_irq[ i ], irq_handlers[i], IRQF_DISABLED, "i2c_pilot_ii", (void *)irq_vadd[i]) < 0 )
#else
         if( request_irq( mapped_irq[ i ], irq_handlers[i], SA_INTERRUPT, "i2c_pilot_ii", NULL ) < 0 )
#endif
#endif
	    {
  ...
  for( i = 0; i < bus_count; i++ )
	...
	irq_icmb = GetIrqFromDT("ami_icmb", IRQ_ICMB);

  
  /* Register SMBus timeout interrupt handler */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,2,0))
  if( request_irq( irq_icmb, i2c_TimeOut_Handler, 0, "i2c_pilot_to", NULL ) < 0 )
#else
  ...
  for( i = 0; i < bus_count; i++ )
   {
     ...
     /* Disable the irq */
     disable_irq( mapped_irq[ i ] );
     
#if (LINUX_VERSION_CODE >=  KERNEL_VERSION(2,6,24)) 
     free_irq( mapped_irq[ i ], (void *)irq_vadd[i]);
#else
     free_irq( mapped_irq[ i ], 0);
#endif
   }
