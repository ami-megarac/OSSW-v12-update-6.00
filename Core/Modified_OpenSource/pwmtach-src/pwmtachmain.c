...
register_pwmtach_hal_module (unsigned char num_instances, void *phal_ops, void **phw_data)
{
	...
	ppwmtach_hal->dev_id = (unsigned int)g_dev_id;
	g_dev_id++;
	ppwmtach_hal->num_fans = (unsigned int)num_instances;
...
configure_fanmap_table (struct pwmtach_dev* pdev, pwmtach_data_t* in_data)
{
	...
	for (i=0; i<in_data->num_fans; i++)
	{
        ...
        pdev->ppwmtach_hal->fan_property_table[i].fan_num = fanmap_data[i].fan_num;
...
configure_fanproperty_table (struct pwmtach_dev* pdev, pwmtach_data_t* in_data)
{
	...
	{
        for (j=0; j < in_data->num_fans; j++ )
        {
            if (pdev->ppwmtach_hal->fan_property_table[i].fan_num == fanproperty_data[j].fan_num)
            {   
		        pdev->ppwmtach_hal->fan_property_table[i].min_rpm = fanproperty_data[j].min_rpm;
		        pdev->ppwmtach_hal->fan_property_table[i].max_rpm = fanproperty_data[j].max_rpm;
		        pdev->ppwmtach_hal->fan_property_table[i].pulses_per_revolution = fanproperty_data[j].pulses_per_revolution;
            }
        }
	}
    show_fanproperty_table(pdev);
	return 0;
}
