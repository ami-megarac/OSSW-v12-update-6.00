...
/* This is a bit of a shameless hack, but, well, it works. */
    if (argc == 3)
    {
        ...
        {
            openlog("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
            /* Reason for False Positive - argv[2] is not user input and buffer unbound is not happens */
            /* coverity[string_size : FALSE] */
            syslog(LOG_ERR, "Incorrect invocation of timeoutd, invalid device(%s).", argv[2]);	/* Fortify [Log Forging]:: False Positive */
            /* Reason for False Positive - argv[2] is not user input and does not contain malicious content */
            closelog();
            exit(5);
        }
		...
		syslog(LOG_NOTICE,
		       ...
		       duplicate, argv[2], config[configline]->daymax);	/* Fortify [Log Forging]:: False Positive */
		/* Reason for False Positive - argv[2] is not user input and does not contain malicious content */
            free(duplicate);
    		closelog();
    		/*
		...
		syslog(LOG_NOTICE,
		       ...
		       duplicate, argv[2]);	/* Fortify [Log Forging]:: False Positive */
		/* Reason for False Positive - argv[2] is not user input and does not contain malicious content */
    		closelog();
            free(duplicate);
    		/*
...
char *in_set;
{
    ...
    while (t)
    {
        ...
        else if (!strncmp(t, element,strlen(t)))
        {
          free(set);
          return 1;
	}
...
int time_remaining;
{
    ...
    closelog();
#endif
    if ((ttyf = fopen(tty, "w")) == NULL)/* Fortify [Path Manipulation]:: False Positive */
    	 /* Reason for False Positive - not accepting user inputs and not using invalid data while using the file */
    {
	openlog("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
        syslog(LOG_ERR, "Could not open %s to warn of impending logoff.\n",tty);	/* Fortify [Log Forging]:: False Positive */
        /* Reason for False Positive - buffer tty is not user input and does not contain malicious content */
        closelog();
        return;
    }
    ...
    switch(chk_timeout(user, dev, host, idle, sesstime))
    {
		...
		openlog("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
    		...
    		else
    		{
    			...
    			if(NULL!=fp){
    				if(-1 == chmod(AuditFile,S_IRWXU|S_IRGRP|S_IRWXO)){	/* Fortify [Race Condition: File System Access]:: False Positive */
    					/* Reason for False Positive - the buffer does not change between the calls of chmod() and fopen() */
    					openlog("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
    					syslog(LOG_NOTICE,
...
/* Tell user which limit they have exceeded and that they will be logged off */
    if ((tty = open(dev, O_WRONLY|O_NOCTTY|O_NONBLOCK)) < 0)/* Fortify [Path Manipulation]:: False Positive */
    	/* Reason for False Positive - not accepting user inputs and not using invalid data while using the file */
    {
	openlog("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
        syslog(LOG_ERR, "Could not write logoff message to %s.", dev); /* Fortify [Log Forging]:: False Positive */
        /* Reason for False Positive - buffer dev is not user input and does not contain malicious content */
        closelog();
	return;
    }
...
/* Wait a little while in case the above message gets lost during logout */
    ...
    if (!kill(pid, 0)) {    /* SIGHUP might be ignored */
        ...
        {
	    openlog("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
            syslog(LOG_ERR, "Could not log user %s off line %s.", user, dev);	/* Fortify [Log Forging]:: False Positive */
            /* Reason for False Positive - buffer user and dev are not user input and does not contain malicious content */
            closelog();
        }
    }
#endif
...
char *d;
{
...
#ifdef linux
    if ((fd = open(d, O_RDONLY|O_NONBLOCK|O_NOCTTY)) < 0)/* Fortify [Path Manipulation]:: False Positive */
    	 /* Reason for False Positive - not accepting user inputs and not using invalid data while using the file */
    {
      openlog("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
      syslog(LOG_WARNING, "Could not open %s for checking line discipline - idle limits will be enforced.", d);	/* Fortify [Log Forging]:: False Positive */
      /* Reason for False Positive - buffer d is not user input and does not contain malicious content */
      closelog();
        return -1;
    }
    ...
    if (ioctl(fd, TIOCGETD, &disc) < 0)
    {
      close(fd);
      openlog("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
      syslog(LOG_WARNING, "Could not get line discipline for %s - idle limits will be enforced.", d); /* Fortify [Log Forging]:: False Positive */
      /* Reason for False Positive - buffer d is not user input and does not contain malicious content */
      closelog();
        return -1;
    }
