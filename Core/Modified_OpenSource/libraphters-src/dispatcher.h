...
void NAME##_func(regmatch_t MATCHES[]) { \
	...
	if (!strncmp(content_type, "application/x-www-form-urlencoded", (sizeof("application/x-www-form-urlencoded") - 1))) { \
...
void NAME##_func(regmatch_t MATCHES[]) { \
    ...
    if(req == NULL){ \
        ...
        json_object_object_add(jresp_nolen, "error", json_object_new_string("Invalid execution sequence")); \
